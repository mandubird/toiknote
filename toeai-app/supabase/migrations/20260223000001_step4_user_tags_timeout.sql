-- STEP 4: 사용자 세부 태그 선택 + 시간 부족 체크
-- SQL Editor에서 STEP 3 마이그레이션 실행 후 이 파일을 실행하세요.

ALTER TABLE public.wrong_answers ADD COLUMN IF NOT EXISTS user_selected_tags JSONB NOT NULL DEFAULT '[]';
ALTER TABLE public.wrong_answers ADD COLUMN IF NOT EXISTS timeout_flag BOOLEAN NOT NULL DEFAULT FALSE;

-- RPC에 user_selected_tags, timeout_flag 인자 추가
...
BEGIN
  ...
  INSERT INTO public.wrong_answers (
    ...
    user_selected_tags, timeout_flag
  ) VALUES (
...
GRANT EXECUTE ON FUNCTION public.save_wrong_note_with_stats(TEXT, INT, TEXT, TEXT, TEXT, TEXT, JSONB, INT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, JSONB, BOOLEAN) TO authenticated;
